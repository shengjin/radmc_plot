...
filename = "numberdens_h12c16o.binp"
print filename
...
head = np.fromfile(filename, count=3, dtype=int)
print "head[0]", head[0]
print "head[1]", head[1]
print "head[2]", head[2]
...
plt.pcolormesh(rad, thet, density_3d[0,:,:], norm=LogNorm())
...
plt.savefig('2d_slice_dens_rad_thet.png')
plt.clf()
...
plt.colorbar()
#r.set_label('Vertical Density of Dust at phi = 0 [ g/cm^2 ]', rotation=90)
plt.savefig('2d_slice_dens_rad_phi.png')
